...
#list the files
onFiles <-
  list.files(ScanOnFolder, recursive = TRUE, full.names = TRUE)
offFiles <-
  list.files(ScanOffFolder, recursive = TRUE, full.names = TRUE)

#how many touch off files we have
cat("\nthere are", length(offFiles), 'files')

# set up an empty dataframe for all of touch off data
scanOff_all_samp1 <- data.frame(
  Mode = integer(),
...
for (i in 1:157) {
  ...
  scanOff_all_samp1 <-
    rbind(as.data.frame(scanOff_all_samp1), as.data.frame(dt))
  
}

colnames(scanOff_all_samp1) <- c(
  'Mode',
  'BusinessDate',
  'DateTime',
  'CardID',
...
BindSampleDataFrames <-
  function(onFiles,
    ...
    for (i in 1:157) {
      ...
      # bind imported file to the main dataframe
      scanOn_all_samp1 <-
        rbind(as.data.frame(scanOn_all_samp1), as.data.frame(dt))
      
    }
    colnames(scanOn_all_samp1) <-
      c(
        'Mode',
        'BusinessDate',
        'DateTime',
    ...
    write.csv(scanOn_all_samp1, file = "all_scanOn_samp1.csv", row.names =
                FALSE)
    write.csv(scanOff_all_samp1, file = "all_scanOff_samp1.csv", row.names =
                FALSE)
  }
...
# TODO: no samp0
# Error: Error in head(all_scanOn_samp0) : object 'all_scanOn_samp0' not found

all_scanOn_samp0 <-
  read_csv("all_scanOn_samp0.csv", col_names = TRUE)
all_scanOff_samp0 <-
  read_csv("all_scanOff_samp0.csv", col_names = TRUE)
all_scanOn_samp1 <-
  read_csv("all_scanOn_samp1.csv", col_names = TRUE)
all_scanOff_samp1 <-
  read_csv("all_scanOff_samp1.csv", col_names = TRUE)
...
car_speeds_separate <-
  separate(
    data = car_speeds,
    col = mean_speed,
    into = paste0("mean_speed_", 1:120),
    sep = ","
  )
car_speeds_separate <-
  separate(
    data = car_speeds_separate,
    col = std_speed,
    into = paste0("std_speed_", 1:120),
    sep = ","
  )
...
for (i in cols_to_change) {
  ...
  if (min_speed == -1) {
    impossible_results_meanspeed <-
      c(impossible_results_meanspeed,
        colnames(car_speeds_separate[i]))
  }
}
...
mapgilbert <-
  get_map(
    location = c(
      lon = mean(max_dataframe$lon),
      lat = mean(max_dataframe$lat)
    ),
    zoom = 9.5,
    maptype = "roadmap",
    scale = 2
...
max_dataframe <-
  data.frame(
    lat = car_speeds_separate$lat,
    lon = car_speeds_separate$lon,
    max_speed = 0,
    colour = '0'
  )
...
for (i in cols_to_change) {
  ...
  if (max_dataframe$max_speed[i] < 20) {
    ...
    #print("20")
  } else if (20 <= max_dataframe$max_speed[i]
             & max_dataframe$max_speed[i] < 40) {
    max_dataframe$colour[i] <-  "#9e6bd5"
    #print("40")
  } else if (40 <= max_dataframe$max_speed[i]
             & max_dataframe$max_speed[i] < 60) {
    max_dataframe$colour[i] <- "#b055ac"
    #print("60")
  } else if (60 <= max_dataframe$max_speed[i]
             & max_dataframe$max_speed[i] < 80) {
    max_dataframe$colour[i] <-  "#b73f82"
    #print("80")
  } else if (80 <= max_dataframe$max_speed[i]
             & max_dataframe$max_speed[i] < 100) {
